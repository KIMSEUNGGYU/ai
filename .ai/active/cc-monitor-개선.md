...
- [x] mock_data 제거 (mock-data.ts + isDemoMode 전체 삭제, prisma 필수화)
- [x] 이벤트 수집 경량화: PreToolUse 제거 + PostToolUse 로컬 카운팅 확대 (PluginHook은 유지)
- [x] 컨벤션 주입 fingerprint: injection_hash(md5) 수집 + History 표시 (파일명, 바이트, 해시)
- [x] 세션 분류 시스템 개선: readActiveTask 상위 디렉토리 탐색 + project_path 폴백 그룹핑
- [x] History 탭 UI 개선: 그룹 아코디언 메인 + 드로어 상세
