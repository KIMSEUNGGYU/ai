...
function isMcpOrPlugin(toolName) {
  return toolName.startsWith("mcp__") || toolName.includes(":");
}

export function isServerRequired(toolName) {
...
export function incrementCounter(sessionId, toolName) {
...
export function readAndClearCounters(sessionId) {
  const cachePath = getCachePath(sessionId);
  try {
